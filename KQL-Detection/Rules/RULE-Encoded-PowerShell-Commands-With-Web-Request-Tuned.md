...
mde_rule_name: "Custom - Encoded PowerShell Commands With Web Request"
sentinel_rule_id: ""
tags:
  - "#detection"
...
# RULE -- Encoded PowerShell Commands With Web Request
...
**Created:** 2026-06-11 | **Status:** Validated
...
Tuned version of  Muid's "Encoded PowerShell Commands With Web Request". Detects base64-encoded PowerShell commands (`-EncodedCommand` / `-enc`) whose decoded content contains web request indicators -- a common pattern for download-and-execute stagers and fileless malware.
...
**Triage checklist:**
1. Is the decoded command a recognizable AI/dev tool OAuth flow (`auth.openai.com`,
   `localhost:<port>/auth/callback`, similar loopback patterns)?
2. Does the decoded command contain an actual retrieval cmdlet AND a subsequent execution
   primitive (`IEX`, `Invoke-Expression`) -- the classic download-and-execute pattern?
3. Is the initiating process a known dev tool (node.exe under JetBrains/VS Code/Cursor paths)
   or an unrecognised process?

If the decoded command combines retrieval + execution and the initiating process is not a
recognised dev tool -- treat as an active stager and escalate. The investigation package
collected by this rule's response action will provide the forensic detail needed.
...
- [ ] Validate tuned query against 30-day lookback for both true positives and remaining FPs
...
<!-- INACTIVE: Sentinel Analytics Rule -- DeviceProcessEvents is Advanced Hunting only -->
...
**Response action rationale:** Collect investigation package was chosen over device
isolation. This rule targets a tuned-but-still-broad pattern (encoded PowerShell +
web indicator + retrieval/execution primitive) that may still have edge-case FPs in
a dev-heavy environment with active AI coding tool adoption. Automatic isolation on
a Medium-severity, recently-tuned rule risks disrupting legitimate developer activity.
Investigation package collection gathers forensic evidence automatically without user
disruption, enabling fast triage. Revisit isolation as a response action once 30-day
post-tuning validation confirms a low FP rate.
...
| Date | Change |
|------|--------|
| 2026-06-11 | Created -- promoted from KQL note. Tuned Microsoft-provided "Encoded PowerShell Commands With Web Request" detection (Option 3: require URL + retrieval/execution primitive together). FP investigated: JetBrains Codex ACP OAuth launch matched on bare `https`. Renamed for Custom- queue convention. Response action: Collect investigation package. |
