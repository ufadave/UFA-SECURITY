...
# KQL — net user password change (HUNT)
...
**Created:** 2026-05-07 | **Status:** `Draft` | **Control Type:** Hunting query
...
This is filed as a **hunting query, not an analytics rule** (`#detection/hunting`), because legitimate help-desk and IT-admin workflows produce a non-trivial false-positive baseline. Promoting to a scheduled rule requires environmental tuning of the IT-admin allowlist (see "Promotion criteria" below).
...
If promoted: change tag from `#detection/hunting` to `#detection/analytics-rule`, set `promoted_to_rule: true`, populate `sentinel_rule_id`. If FP rate stays high after tuning, **keep this as a hunting query** — consistent with the principle that detection-primitive fit to environment matters more than coverage breadth.
